...
| **Fase 0, empezada (25-sep-2026)** | `guajirago-pruebas` existe, y la app de transporte (`b1c8a63`) y el panel (`0664217` de `guajirago-admin`) compilan por ambiente: `npm run build:pruebas` / `build:produccion`, cartel PRUEBAS, `.firebaserc` con PRUEBAS por defecto (el panel publica en `guajirago-pruebas-admin.web.app`). Faltan: la base de datos y el plan Blaze de pruebas (consola, del dueño), reglas, índices y funciones a pruebas, el guion de siembra, y aliados. Lo cuenta `node scripts/medir-ambientes.cjs` |
