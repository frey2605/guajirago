...
| **Fase 0, empezada (25-sep-2026)** | `guajirago-pruebas` existe, y las tres apps compilan por ambiente: transporte (`b1c8a63`), el panel (`0664217` de `guajirago-admin`) y aliados (`30cd8d1` de `guajirago-aliados`): `npm run build:pruebas` / `build:produccion`, cartel PRUEBAS, `.firebaserc` con PRUEBAS por defecto; en pruebas publican en `guajirago-pruebas.web.app`, `guajirago-pruebas-admin.web.app` y `guajirago-pruebas-aliados.web.app`. Ya tiene base de datos (nam5), Auth, Blaze, las reglas y los 3 índices del repo (publicados el 25-sep desde copia limpia de `7ef29a2`, leídos de vuelta: idénticos) y llave de notificaciones. Faltan: activar Firebase Storage (consola, del dueño), reglas del almacén y funciones a pruebas, los dos `firebase-messaging-sw.js` (llevan producción a mano) y volver a publicar transporte de pruebas, y el guion de siembra. Lo cuenta `node scripts/medir-ambientes.cjs` |
