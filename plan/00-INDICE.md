...
| **Fase 0, empezada (25-sep-2026)** | `guajirago-pruebas` existe, y las tres apps compilan por ambiente: transporte (`b1c8a63`), el panel (`0664217` de `guajirago-admin`) y aliados (`30cd8d1` de `guajirago-aliados`): `npm run build:pruebas` / `build:produccion`, cartel PRUEBAS, `.firebaserc` con PRUEBAS por defecto; en pruebas publican en `guajirago-pruebas.web.app`, `guajirago-pruebas-admin.web.app` y `guajirago-pruebas-aliados.web.app`. Faltan: la base de datos y el plan Blaze de pruebas (consola, del dueño), reglas, índices y funciones a pruebas, y el guion de siembra. Lo cuenta `node scripts/medir-ambientes.cjs` |
