...
| — | ✅ **CERRADO** (24-sep) — nadie vigilaba las citas de la carpeta `plan/` | `scripts/medir-citas.cjs`, su lista de carpetas | Saboteado: se metió aquí una cita a un guion que no existe y **nada se puso rojo**. El medidor no entraba en `plan/`; y metiéndolo tal cual daba 53 falsas alarmas, porque exigía que el nombre empezara por letra y los de `plan/` empiezan por `01-`. Ahora entra, lee los nombres con número, comprueba también las citas escritas `plan/…`, y **dice cuántas notas `.md` del repo se quedan sin mirar**: el amarre exige cero. **Lo que sigue sin ver:** una carpeta solo de código (hoy `guajirago-publicidad/`, un archivo) |
