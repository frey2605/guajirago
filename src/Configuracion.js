...
function Configuracion({ onVolver, onCerrarSesion }) {
  ...
  const toggle = (activo, onClick) => (
    <div onClick={onClick} style={{ width: '52px', height: '30px', borderRadius: '15px', background: activo ? 'linear-gradient(135deg, #FFCF4D, #FF7A2F)' : '#C9CDD3', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: '0 4px', justifyContent: activo ? 'flex-end' : 'flex-start', flexShrink: 0, transition: 'all 0.2s' }}>
      <div style={{ width: '22px', height: '22px', borderRadius: '50%', background: '#FFFFFF' }} />
    </div>
  );

  const filaToggle = (icono, texto, activo, onClick) => (
    <div style={{ background: '#FFFFFF', border: '1.5px solid #ECECEF', borderRadius: '14px', padding: '14px 16px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '12px' }}>
      <span style={{ fontSize: '20px' }}>{icono}</span>
      <span style={{ color: '#1A1A1E', fontSize: '15px', fontWeight: '500', flex: 1 }}>{texto}</span>
      {toggle(activo, onClick)}
    </div>
  );

  const filaBoton = (icono, texto, onClick, color) => (
    <div onClick={onClick} style={{ background: '#FFFFFF', border: '1.5px solid #ECECEF', borderRadius: '14px', padding: '14px 16px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}>
      <span style={{ fontSize: '20px' }}>{icono}</span>
      <span style={{ color: color || '#1A1A1E', fontSize: '15px', fontWeight: '500', flex: 1 }}>{texto}</span>
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', minHeight: '100vh', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ padding: '16px 20px' }}>
        ...
        <p style={{ color: '#6B7280', fontSize: '11px', letterSpacing: '3px', margin: '0 0 8px' }}>NOTIFICACIONES</p>
        ...
        <div style={{ background: '#FFFFFF', border: '1.5px solid #ECECEF', borderRadius: '14px', padding: '10px 16px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '20px' }}>🌍</span>
          <span style={{ color: '#1A1A1E', fontSize: '15px', fontWeight: '500', flex: 1 }}>Idioma</span>
          <span style={{ color: '#6B7280', fontSize: '14px' }}>Español</span>
        </div>

        <p style={{ color: '#6B7280', fontSize: '11px', letterSpacing: '3px', margin: '16px 0 8px' }}>LEGAL</p>
        {filaBoton('📄', 'Términos y condiciones', () => setVerTerminos(true))}
        {filaBoton('🔒', 'Política de privacidad', () => setVerPrivacidad(true))}

        <p style={{ color: '#6B7280', fontSize: '11px', letterSpacing: '3px', margin: '16px 0 8px' }}>ACERCA DE</p>
        <div style={{ background: '#FFFFFF', border: '1.5px solid #ECECEF', borderRadius: '14px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '20px' }}>📱</span>
          <span style={{ color: '#1A1A1E', fontSize: '15px', fontWeight: '500', flex: 1 }}>Versión de la app</span>
          <span style={{ color: '#6B7280', fontSize: '14px' }}>{VERSION_APP}</span>
        </div>

        <p style={{ color: '#6B7280', fontSize: '12px', textAlign: 'center', margin: '24px 0 0' }}>GuajiraGo © 2026 · Riohacha, La Guajira</p>
      </div>

      {confirmarEliminar && (
        <div onClick={() => { setConfirmarEliminar(false); setContrasenaEliminar(''); setErrorEliminar(''); }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#FFFFFF', borderRadius: '24px', padding: '32px 24px', width: '100%', maxWidth: '400px', border: '2px solid #FF4444', textAlign: 'center' }}>
            <div style={{ fontSize: '54px', marginBottom: '12px' }}>⚠️</div>
            <h2 style={{ color: '#1A1A1E', fontSize: '20px', fontWeight: '900', margin: '0 0 10px' }}>¿Eliminar tu cuenta?</h2>
            <p style={{ color: '#6B7280', fontSize: '14px', margin: '0 0 16px', lineHeight: '1.5' }}>Esta acción es permanente. Se borrarán todos tus datos y no podrás recuperarlos. Escribe tu contraseña para confirmar.</p>
            <div style={{ background: '#FFFFFF', borderRadius: '14px', padding: '14px 16px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '12px', border: `1px solid ${errorEliminar ? '#FF4444' : '#ECECEF'}` }}>
