...
function Solicitar({ tipo, onVolver, destinoInicial }) {
  ...
  const fnCrearViaje = httpsCallable(functions, 'crearViaje');
  const fnSubirTarifa = httpsCallable(functions, 'subirTarifa');
  const fnAceptarOferta = httpsCallable(functions, 'aceptarOferta');
  const fnRechazarOferta = httpsCallable(functions, 'rechazarOferta');
  const fnCancelarViaje = httpsCallable(functions, 'cancelarViaje');
  ...
  const cancelarViaje = async (razon) => {
    clearInterval(contadorRef.current);
    if (viajeId) await fnCancelarViaje({ viajeId, canceladoPor: 'pasajero', razonCancelacion: razon }).catch(() => {});
  ...
  const solicitarViaje = async () => {
    ...
    try {
      ...
      const result = await fnCrearViaje({
      ...
      setViajeId(result.data.viajeId);
      setContraofertas([]);
      contaofertasIdsRef.current.clear();
  ...
  const aceptarContraoferta = async (oferta) => {
    ...
    setCelebrando(true);
    try {
      await fnAceptarOferta({ viajeId, conductorId: oferta.conductorId });
    } catch (e) {}
    setTimeout(() => {
      setCelebrando(false);
      setPantalla('fase1');
  ...
  const rechazarContraoferta = async (conductorId) => {
    ...
    if (viajeId) {
      fnRechazarOferta({ viajeId, conductorId }).catch(() => {});
    }
  };
