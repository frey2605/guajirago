...
function MapaPasajero({ ubicacionPasajero, ubicacionConductor, tipo }) {
  ...
  // Marcador del pasajero (se actualiza si llega su ubicación después)
  useEffect(() => {
  ...
  // Calcular la ruta cada vez que tengamos ambos puntos
  useEffect(() => {
    ...
    directionsService.route({
      ...
      if (status === 'OK') {
        ...
        rutaRef.current = new window.google.maps.DirectionsRenderer({
          ...
          polylineOptions: { strokeColor: '#2ECC71', strokeWeight: 5 },
        });
  ...
  }, [ubicacionConductor, ubicacionPasajero]);
...
function Solicitar({ tipo, onVolver }) {
  ...
  useEffect(() => {
    ...
    // Intento rápido sin alta precisión (iPhone responde al instante)
    navigator.geolocation.getCurrentPosition(
      (pos) => setUbicacionPasajero({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
      () => {
        // Respaldo con alta precisión
        navigator.geolocation.getCurrentPosition(
          (pos) => setUbicacionPasajero({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
          ...
          <div style={{ position: 'absolute', top: '90px', left: '16px', right: '16px', zIndex: 10, background: 'rgba(20,20,22,0.97)', borderRadius: '20px', padding: '20px', border: '2px solid #2ECC71' }}>
            ...
            <div style={{
              ...
              <p style={{ color: '#555', fontSize: '12px', margin: '0' }}>
                {contador === 0 ? '⚠️ Tiempo agotado' : 'Sal pronto o el conductor puede cancelar'}
              </p>
              <p style={{
                color: contador <= 60 ? '#FF4444' : '#FFCF4D',
                fontSize: '28px', fontWeight: '900', margin: '0', fontVariantNumeric: 'tabular-nums',
              }}>
  ...
  if (pantalla === 'fase2') {
    return (
      <div style={{ backgroundColor: '#141416', minHeight: '100vh', position: 'relative' }}>
        <MapaPasajero ubicacionPasajero={ubicacionPasajero} ubicacionConductor={ubicacionConductor} tipo={tipo} />
        <div style={{ position: 'absolute', top: '16px', left: '16px', right: '16px', zIndex: 10, background: 'rgba(20,20,22,0.95)', borderRadius: '16px', padding: '12px 16px' }}>
  ...
  if (pantalla === 'esperando') {
    return (
      <div style={{ backgroundColor: '#141416', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        ...
        <button onClick={onVolver} style={{ background: 'transparent', border: '1px solid #2A2A2E', borderRadius: '14px', color: '#555', fontSize: '14px', padding: '14px 32px', cursor: 'pointer' }}>Cancelar</button>
      </div>
    );
  }
