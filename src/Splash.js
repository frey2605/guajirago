...
function Splash({ onFinish }) {
  ...
  const instalarAndroid = async () => {
    if (deferredPrompt) {
      ...
      if (resultado.outcome === 'accepted') onFinish();
    }
  };

  const saltarInstalacion = () => onFinish();
      ...
      <div style={{ width: '100%', marginBottom: '32px' }}>
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', borderBottom: '1px solid #E2E8F0' }}>
            ...
            <p style={{ color: '#0A0A0A', fontSize: '15px', margin: '0' }}>{item.texto}</p>
          </div>
        ))}
      </div>

      {esIphone && (
        <div style={{ background: '#F5F7FA', borderRadius: '20px', padding: '20px', width: '100%', marginBottom: '16px', border: '1px solid #E2E8F0' }}>
          <p style={{ color: '#0A0A0A', fontSize: '15px', fontWeight: 'bold', margin: '0 0 12px', textAlign: 'center' }}>📲 Cómo instalar en iPhone:</p>
          <p style={{ color: '#6B7280', fontSize: '13px', margin: '0 0 6px' }}>1️⃣ Toca <strong style={{ color: '#1A3ED4' }}>↑ Compartir</strong> abajo en Safari</p>
          <p style={{ color: '#6B7280', fontSize: '13px', margin: '0' }}>2️⃣ Toca <strong style={{ color: '#1A3ED4' }}>"Añadir a pantalla de inicio"</strong></p>
        </div>
      )}

      {!esIphone && deferredPrompt && (
        <button onClick={instalarAndroid} style={{ width: '100%', padding: '18px', background: 'linear-gradient(135deg, #4D6FE8, #1A3ED4, #0F2498)', border: 'none', borderRadius: '16px', color: '#FFFFFF', fontSize: '18px', fontWeight: '900', cursor: 'pointer', marginBottom: '12px' }}>
          📲 Instalar GuajiraGo ahora
        </button>
      )}

      <button onClick={saltarInstalacion} style={{ background: 'none', border: 'none', color: '#6B7280', fontSize: '14px', cursor: 'pointer', marginTop: '8px' }}>
