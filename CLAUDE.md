...
|---|---|---|
| 🔴 **EL BOTÓN DE PÁNICO DE VERDAD SE CALLA** | `guajirago/src/Solicitar.js:772` `compartirSeguridad()` | **es el que de verdad se aprieta**: el 🚨 rojo que flota sobre el mapa durante el viaje (`:1199`, `:1298` → `PanelEmergencia` → `:1113`). Arma su mensaje A MANO, aparte del de Ajustes, y tiene CUATRO fallos, tres de ellos silencios: `:503` (carga el contacto de confianza con un `catch (e) {}` MUDO — si falla, el botón abre WhatsApp **sin destinatario** y el pasajero no sabe por qué), `:775` (sin ubicación no dice nada; el de Ajustes sí lo dice), `:779` (el encabezado «MI RUTA» sale aunque no haya ni origen ni destino) y `:783` (**manda los datos del conductor comprobando solo `if (viaje)`, sin mirar `viaje.conductorId`** — es EXACTAMENTE lo que el dueño mandó arreglar el 11-sep con las palabras «tu ruta sí, conductor no», y aquí sigue puesto). El 12-sep-2026 se arregló el de **Ajustes** y éste quedó igual: unificarlo es trabajo aparte, con sus 12 pasos y permiso del dueño. **Lo cazó la segunda opinión — yo lo llamé «el botón de pánico» al otro todo el día, y ése fue el motivo de que este no se viera.** |
...
| El nombre falso, en dos sitios más | `scripts/medir-panico.cjs` · `pruebas/amarres.test.js:1333` y `:1348` | siguen llamando «EL BOTÓN DE PÁNICO» a la pantalla de Ajustes. Son del trabajo del 11-sep, ya commiteado, así que no se tocaron de paso (PRIMERA LEY). Es un comentario que miente, y ya escondió un fallo una vez |
