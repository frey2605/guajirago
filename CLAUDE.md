...
| El conteo «en curso» de mensajería | `guajirago-admin/src/Mensajeria.js`, `esEnCurso` | cuenta con `['esperando', 'aceptado', 'en_viaje']`, y `'en_viaje'` **no es un estado, es una fase**: ningún viaje lo tiene en `estado`, así que ese tercero no suma nada. Tiene que salir de `ESTADOS_EN_CURSO` (`guajirago/src/estadosViaje.js`), que es la fuente única. ⚠️ **Esta fila decía además «y SIN `en_negociacion`»: eso ya no vale** — ese estado se retiró el 12-sep-2026, así que la nota mandaba a añadir un estado muerto. Y **el comentario del propio archivo lo sigue diciendo**: `guajirago-admin/src/Mensajeria.js`, justo encima de `esEnCurso`. No se tocó porque ese repo no estaba en la foto (PRIMERA LEY): hay que arreglarlo **al hacer este trabajo**, porque quien abra el archivo lee ese comentario, no esta tabla |
