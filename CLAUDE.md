...
| 🔴 **LOS 9 VIAJES `expirado` NO SALEN EN NINGUNA PESTAÑA DEL PANEL** | `guajirago-admin/src/Viajes.js` (la pestaña de cancelados) y `App.js` | «en curso» consulta `esperando`/`aceptado`, «completados» filtra `finalizado`, y «cancelados» solo `cancelado` y `cancelado_conductor`. Los **9 viajes en `expirado`** —los que el conductor aceptó y dejó a medias— **desaparecen del panel**: no están en ninguna caja. Es el mismo fallo que se cerró el 12-sep-2026 en la pantalla de mensajería, pero MÁS GRANDE (9 viajes, no 5 mandados). Lo encontró la segunda opinión mirando si había gemelos. El amarre nuevo cubre mensajería, **no éste**. La lista buena es `ESTADOS_TERMINADOS` |
