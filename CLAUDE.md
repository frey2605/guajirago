...
| 🔴 **5 DE 12 MANDADOS SE VEN «EN CURSO» ESTANDO CANCELADOS** | `guajirago-admin/src/Mensajeria.js`: `esCancelado` y el cajón de sastre de `etiquetaEstado` | `esCancelado` solo conoce `['cancelado', 'vencido']`. Los mandados en `expirado` (los cierra la rutina nocturna) y en `cancelado_conductor` **no caben en ninguna caja**, así que caen en el `return { t: 'En curso' }` del final y se pintan **en naranja como «En curso»** en Historial y Buscar — a la vez que **no se cuentan** ni en «EN CURSO AHORA» ni en «CANCELADOS HOY». Medido el 12-sep-2026: **5 de los 12 mandados** (2 `expirado`, 3 `cancelado_conductor`). La lista buena ya existe: `ESTADOS_TERMINADOS` en `guajirago/src/estadosViaje.js`. Se cuenta con `node scripts/medir-mandados.cjs`. **Apareció al arreglar `esEnCurso`, dos renglones más arriba; se ANOTA y no se toca (PRIMERA LEY)** |
