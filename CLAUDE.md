...
| 🔴 **EL MISMO FALLO, EN OTRAS SIETE PANTALLAS** | `guajirago-admin/`: `App.js` (las gráficas de cancelados del tablero), `Pasajeros.js` (la ficha del pasajero), el desplegable de estados del buscador de `Viajes.js` · `guajirago/`: `AppConductor.js` (el historial del conductor y su etiqueta), `Home.js` y `MisViajes.js` | listas de estados incompletas que hacen desaparecer registros, igual que la pestaña que se cerró el 12-sep-2026. Lo más gordo, medido ese día: **el historial del conductor** (`AppConductor.js`) filtra `finalizado \|\| cancelado`, así que se deja fuera **31 `cancelado_conductor` + 9 `expirado`**; y tres sitios pintan `cancelado` a secas, así que **un viaje que canceló el conductor se ve como si hubiera terminado bien**. El desplegable del buscador solo ofrece `esperando`, `finalizado`, `cancelado` y `cancelado_conductor`: los 9 `expirado` salen en la pestaña pero **no se pueden buscar**. La lista buena es `ESTADOS_TERMINADOS`; la forma de arreglarlo está hecha en `Viajes.js` (`NO_COMPLETADOS` + amarre). **Se ANOTA y no se toca: son 5 archivos de 2 repos que no estaban en la foto (PRIMERA LEY)** |
...
| El historial del pasajero se deja 40 viajes fuera | `guajirago/src/Home.js` y `MisViajes.js` | «Mis viajes» filtra `finalizado \|\| cancelado`, así que los `cancelado_conductor` (31), `expirado` (9) y `vencido` **no aparecen nunca**. El pasajero no ve los viajes que le cancelaron o que se quedaron colgados |
